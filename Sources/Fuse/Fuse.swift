...
public class Fuse {
    ...
    private let searchQueue = DispatchQueue(label: "fuse.search.queue", attributes: .concurrent)
    ...
    public func search(
        ...
        if tokenize {
            ...
            let averagedResult = (
                ...
                ranges: Array(Set(results.ranges))
            )
...
extension Fuse {
    ...
    private func _search(
        ...
        let text = isCaseSensitive ? aString : aString.lowercased()

        let textCount = text.count

        // Exact match
        if pattern.text == text {
            return (0, [0...textCount - 1])
        }
        ...
        var bestLocation: Int? = if let index = text.index(of: pattern.text, startingFrom: location) {
        ...
        var matchMaskArr = [Int](repeating: 0, count: textCount)

        // Get all exact matches, here for speed up
        var index = text.index(of: pattern.text, startingFrom: bestLocation)
        while let _index = index {
            let i = text.distance(from: text.startIndex, to: _index)
            let score = FuseUtilities.calculateScore(pattern.len, e: 0, x: i, loc: location, distance: distance)
        ...
        var score = 1.0
        var binMax = pattern.len + textCount
        var lastBitArr = [Int]()
        ...
        for i in 0..<pattern.len {
            ...
            let finish = min(location + binMid, textCount) + pattern.len
            ...
            for j in (start...finish).reversed() {
                ...
                let charMatch: Int = {
                    if currentLocation < textCount {
                        let _currentLocationIndex = currentLocationIndex.map{ text.index(before: $0)}
                        ?? text.index(text.startIndex, offsetBy: currentLocation)

                        currentLocationIndex = _currentLocationIndex
                        let char = text[_currentLocationIndex]
...
extension Fuse {
    ...
    public func search(_ text: String, in aList: [String]) -> [SearchResult] {
        ...
        aList.enumerated().forEach { index, item in
    ...
    public func search(
        ...
        stride(from: 0, to: count, by: chunkSize).forEach { offset in
            ...
            searchQueue.async {
                ...
                chunk.enumerated().forEach { index, item in
    ...
    public func search(_ text: String, in aList: [Fuseable]) -> [FusableSearchResult] {
        ...
        for (index, item) in aList.enumerated() {
            ...
            item.properties.forEach { property in
                ...
                if let result = search(pattern, in: value) {
